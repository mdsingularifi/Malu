...
pub async fn create_secret(
    State(service): State<Arc<SecretService>>,
    ...
    match service.store_secret(&request.path, namespace, &request.data, username).await {
        Ok(_) => {
...
pub async fn get_secret(
    ...
    let username: Option<&str> = None;
    match service.retrieve_secret(&path, namespace, username).await {
        Ok(data) => {
...
pub async fn delete_secret(
    ...
    let namespace = "default";
    let username: Option<&str> = None;
    match service.delete_secret(&path, namespace, username).await {
        Ok(_) => {
...
pub async fn list_secrets(
    ...
    let username: Option<&str> = None;
    match service.list_secrets(namespace, prefix, username).await {
        Ok(paths) => {
