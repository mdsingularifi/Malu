...
//!
//! This module provides integration with hardware security modules (HSMs)
...
impl HsmCryptoProvider {
            ...
            HsmType::Pkcs11 => {
                #[cfg(feature = "pkcs11")]
                {
                    let lib_path = self.config.pkcs11_lib_path.as_ref()
                        .ok_or_else(|| CryptoError::Hsm("PKCS#11 library path not specified".into()))?;
                    
                    debug!("Initializing PKCS#11 with library: {}", lib_path);
                    
                    let ctx = Ctx::new(lib_path)
                        .map_err(|e| CryptoError::Hsm(format!("Failed to initialize PKCS#11: {:?}", e)))?;
                    
                    ctx.initialize(None)
                        .map_err(|e| CryptoError::Hsm(format!("Failed to initialize PKCS#11 context: {:?}", e)))?;
                ...
                {
                    return Err(CryptoError::Hsm("PKCS#11 support not enabled".into()));
                }
            },
            HsmType::YubiKey => {
                #[cfg(feature = "yubikey")]
                ...
                {
                    return Err(CryptoError::Hsm("YubiKey support not enabled".into()));
                }
            },
            HsmType::Tpm => {
                debug!("TPM support not yet implemented");
                return Err(CryptoError::Hsm("TPM support not yet implemented".into()));
            }
        }
    ...
    fn get_session(&mut self) -> Result<Session> {
        let ctx = self.ctx.as_ref()
            .ok_or_else(|| CryptoError::Hsm("PKCS#11 not initialized".into()))?;
        
        let slot_id = self.config.slot_id
            .ok_or_else(|| CryptoError::Hsm("Slot ID not specified".into()))?;
        
        let slot_id = SlotId::from(slot_id);
        let pin = self.config.pin.as_ref()
            .ok_or_else(|| CryptoError::Hsm("PIN not specified".into()))?;
        
        let session = ctx.open_session(slot_id, pkcs11::types::CKF_SERIAL_SESSION | pkcs11::types::CKF_RW_SESSION, None, None)
            .map_err(|e| CryptoError::Hsm(format!("Failed to open session: {:?}", e)))?;
        
        session.login(pkcs11::types::CKU_USER, Some(pin.as_bytes()))
            .map_err(|e| CryptoError::Hsm(format!("Failed to login: {:?}", e)))?;
    ...
    fn find_key(&mut self, key_id: &str) -> Result<Object> {
        ...
        session.find_objects(&template)
            .map_err(|e| CryptoError::Hsm(format!("Failed to find objects: {:?}", e)))?
            .into_iter()
            .next()
            .ok_or_else(|| CryptoError::Key(format!("Key with ID {} not found", key_id)))
    }
    ...
    fn generate_key(&mut self, key_id: &str, key_len: usize) -> Result<Object> {
        ...
        session.generate_key(&mechanism, &template)
            .map_err(|e| CryptoError::Hsm(format!("Failed to generate key: {:?}", e)))
    }
}
    ...
    fn drop(&mut self) {
        ...
        if let Some(ctx) = &self.ctx {
            ...
            if let Err(e) = ctx.finalize() {
                error!("Failed to finalize PKCS#11 context: {:?}", e);
            }
        }
    }
}
...
impl CryptoProviderInterface for HsmCryptoProvider {
    async fn encrypt(&self, context: &str, plaintext: &[u8], key: &[u8]) -> std::result::Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        #[cfg(not(feature = "pkcs11"))]
        {
            return Err(Box::new(CryptoError::Hsm("PKCS#11 support not enabled".into())));
        }
    }

    async fn decrypt(&self, context: &str, ciphertext: &[u8], key: &[u8]) -> std::result::Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn derive_key(&self, passphrase: &[u8], salt: &[u8], info: &[u8]) -> std::result::Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        #[cfg(feature = "pkcs11")]
        {
            ...
            // For now, we'll use a software fallback
            use crate::kdf::{derive_key, KdfParams, KdfAlgorithm};
    ...
    async fn generate_random(&self, length: usize) -> std::result::Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        #[cfg(feature = "pkcs11")]
        {
            ...
            session.generate_random(&mut bytes)
                .map_err(|e| Box::new(CryptoError::Hsm(format!("Failed to generate random: {:?}", e))) as Box<dyn std::error::Error + Send + Sync>)?;
    ...
    async fn hash(&self, data: &[u8]) -> std::result::Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn sign(&self, message: &[u8], key: &[u8]) -> std::result::Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        #[cfg(feature = "pkcs11")]
        {
            ...
            // For now, we'll use a software fallback
            use crate::signature::{sign, SignatureAlgorithm};
            ...
            let key_pair = crate::signature::generate_key_pair(SignatureAlgorithm::Ed25519)?;
    ...
    async fn verify(&self, message: &[u8], signature: &[u8], key: &[u8]) -> std::result::Result<bool, Box<dyn std::error::Error + Send + Sync>> {
        ...
        #[cfg(feature = "pkcs11")]
        {
            ...
            // For now, we'll use a software fallback
            use crate::signature::{verify, SignatureAlgorithm};
...
impl Clone for HsmCryptoProvider {
    fn clone(&self) -> Self {
        // We don't clone the actual HSM context, just the configuration
        Self {
...
mod tests {
    ...
    async fn test_hsm_random() {
        let config = HsmConfig {
            hsm_type: HsmType::Pkcs11,
            pkcs11_lib_path: Some("/path/to/pkcs11/lib.so".to_string()),
            slot_id: Some(0),
            pin: Some("1234".to_string()),
            token_label: Some("MALU-HSM".to_string()),
            key_id: Some("test-key".to_string()),
        };
        
        let mut provider = HsmCryptoProvider::new(&config);
        provider.initialize().unwrap();
        
        let random = provider.generate_random(32).await.unwrap();
